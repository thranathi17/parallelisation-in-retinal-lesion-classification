...
# Ignore InceptionV3 Model Warning
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def _find_classes(self, dir):
        """
        Finds the class names and maps them to indices in the dataset directory.
    ...
    def make_dataset(self):
        """
        Creates a dataset list containing image paths and their corresponding class indices.
    ...
    def __getitem__(self, idx):
        """
        Fetches the image and its label at a given index.

        Args:
            idx (int): Index of the data item.

        Returns:
            tuple: Contains the transformed image tensor, its class index, and the image path.
        """
...
# Prepares PyTorch DataLoader with a DistributedSampler to efficiently distribute the data across multiple GPUs with 4 workers.
def prepare_dataloader(dataset, batch_size, world_size, rank, num_workers=4):
    """
...
# Initializes the InceptionV3 model with custom modifications for the final fully connected layer
# to adapt to the number of classes in the dataset.
# It also sets the specified layers to be trainable.
def initialize_model(num_classes=4):
    """
    Initializes and customizes the InceptionV3 model for the specific dataset.

    Args:
        num_classes (int, optional): The number of classes in the dataset.

    Returns:
        model (torch.nn.Module): The modified InceptionV3 model.
    """
...
def train_model(
        ...
        for phase in ["train", "val"]:
            ...
            for inputs, labels, _ in data_iter:
                ...
                with torch.set_grad_enabled(phase == "train"):
                    ...
                    if phase == "train":
                        # Separate main and auxiliary outputs for Inception model
                        main_output, aux_output = outputs
                        loss = criterion(main_output, labels) + 0.4 * criterion(
                            aux_output, labels
                        )
    ...
    if rank == 0:
        if world_size == 1:
            torch.save(
                best_model_wts,
                f"InceptionV3_4W/inception_model_best_{world_size}g_4w.pth",
            )
        print(
        ...
        with open(f"InceptionV3_4W/training_metrics_{world_size}g_4w.json", "w") as f:
            json.dump(metrics, f)
...
def evaluate_model(model, test_loader, device):
    ...
    # Save to JSON file
    with open("InceptionV3_4W/model_predictions_4w.json", "w") as f:
    ...
    try:
        ...
        if rank == 0 and world_size == 1:
            model.load_state_dict(
                torch.load(f"InceptionV3_4W/inception_model_best_{world_size}g_4w.pth")
            )
