...
def train_model(
    ...
    if rank == 0:
        if world_size == 1:
            torch.save(
                best_model_wts,
                f"InceptionV3_8W/inception_model_best_{world_size}g_8w.pth",
            )
        print(
        ...
        with open(f"InceptionV3_8W/training_metrics_{world_size}g_8w.json", "w") as f:
            json.dump(metrics, f)
...
def evaluate_model(model, test_loader, device):
    ...
    # Save to JSON file
    with open("InceptionV3_8W/model_predictions_8w.json", "w") as f:
    ...
    try:
        ...
        if rank == 0 and world_size == 1:
            model.load_state_dict(
                torch.load(f"InceptionV3_8W/inception_model_best_{world_size}g_8w.pth")
            )
