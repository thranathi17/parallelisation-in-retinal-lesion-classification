...
def train_resnet_model(
    ...
    if rank == 0:
        if world_size == 1:
            torch.save(
                best_model_wts, f"ResNet50_4W/resnet_model_best_{world_size}g_4w.pth"
            )
        print(
        ...
        with open(f"ResNet50_4W/training_metrics_{world_size}g_4w.json", "w") as f:
            json.dump(metrics, f)
...
def evaluate_model(model, test_loader, device):
    ...
    # Save to JSON file
    with open("ResNet50_4W/model_predictions_4w.json", "w") as f:
    ...
    try:
        ...
        if rank == 0 and world_size == 1:
            model.load_state_dict(
                torch.load(f"ResNet50_4W/resnet_model_best_{world_size}g_4w.pth")
            )
