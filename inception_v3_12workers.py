...
def train_model(
    ...
    if rank == 0:
        if world_size == 1:
            torch.save(
                best_model_wts,
                f"InceptionV3_12W/inception_model_best_{world_size}g_12w.pth",
            )
        print(
        ...
        with open(f"InceptionV3_12W/training_metrics_{world_size}g_12w.json", "w") as f:
            json.dump(metrics, f)
...
def evaluate_model(model, test_loader, device):
    ...
    # Save to JSON file
    with open("InceptionV3_12W/model_predictions_12w.json", "w") as f:
